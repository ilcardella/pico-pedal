...
*/
SSD1306::SSD1306(uint16_t const DevAddr, uint8_t const width, uint8_t const height,
                 i2c_inst_t *i2c)
    : DevAddr(DevAddr), width(width), height(height), i2c(i2c)
{
...
void SSD1306::rotateDisplay(uint8_t Rotate)
{
    ...
    this->sendCommand(0xA0 |
                      (0x01 & Rotate)); // Set Segment Re-Map Default
    ...
    this->sendCommand(
        ...
        (0x08 & (Rotate << 3))); // Set COM Output Scan Direction
                                 // 0xC0	(0x00) => normal mode (RESET) Scan from COM0
                                 // to COM[N-1];Where N is the Multiplex ratio. 0xC8
